    ...
    const columns: ColumnDef<ICoachPublic>[] = useMemo(
        () => [
            ...
            },
            {
                ...
                cell: ({ row }: CellContext<ICoachPublic, unknown>) =>
                    row.original.username ?? (
                        <Tooltip title="Login mobile pakai nomor telepon">
                            <span className="text-gray-400">via telepon</span>
                        </Tooltip>
                    ),
            },
            {
            ...
            },
            {
                ...
                cell: ({ row }: CellContext<ICoachPublic, unknown>) =>
                    row.original.tarif_per_sesi !== null ? (
                        formatRupiah(row.original.tarif_per_sesi)
                    ) : (
            ...
            },
            {
                ...
                size: 160,
                cell: ({ row }: CellContext<ICoachPublic, unknown>) =>
                    row.original.spesialisasi ?? <span className="text-gray-400">—</span>,
            },
            {
                header: 'Status',
                accessorKey: 'aktif',
