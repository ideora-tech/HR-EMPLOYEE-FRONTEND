...
## Modul Kursus — Presensi & Catat Kelas Siswa
...
└── kursus/
    └── presensi/
        ├── page.tsx                        ← Halaman kalender absen (tampilan utama)
        └── [id_jadwal]/
            └── page.tsx                    ← Absen massal satu sesi jadwal

components/
└── kursus/
    └── presensi/
        ├── PresensiKalender.tsx            ← Grid kalender hari (Sen–Min) per instruktur
        ├── PresensiSesiCard.tsx            ← Card jadwal di dalam kalender
        ├── AbsenSesiModal.tsx              ← Modal absen massal (batch upsert)
        ├── AbsenSiswaRow.tsx               ← Satu baris siswa di dalam modal absen
        ├── StatusBadge.tsx                 ← Badge warna status (HADIR/TIDAK HADIR/SAKIT/IZIN)
        └── CatatKelasSiswaPanel.tsx        ← Panel ringkasan sesi hadir siswa per kelas

services/
├── presensi.service.ts                     ← Semua API call presensi
└── catat-kelas-siswa.service.ts            ← API call catat kelas siswa (GET only)
```

---

### Types — `src/@types/presensi.types.ts`
...
export type PresensiStatus = typeof PRESENSI_STATUS[keyof typeof PRESENSI_STATUS]

export const PRESENSI_STATUS_LABEL: Record<PresensiStatus, string> = {
  1: 'Hadir',
  2: 'Tidak Hadir',
  3: 'Sakit',
  4: 'Izin',
...
export const PRESENSI_STATUS_COLOR: Record<PresensiStatus, string> = {
  1: 'success',   // hijau
  2: 'error',     // merah
  3: 'warning',   // kuning
  4: 'info',      // biru
...
export interface IJadwalPresensi {
...
export interface ISiswaPresensi {
  id_siswa: string
  nama_siswa: string
  email: string | null
  telepon: string | null
...
export interface IPresensi {
  ...
  waktu_mulai_kelas: string | null
  catatan: string | null
  aktif: number
  dibuat_pada: string
  diubah_pada: string | null
  jadwal: IJadwalPresensi
  siswa: ISiswaPresensi
}

// Item list absen per jadwal (untuk tampilan absen massal)
export interface IPresensiJadwalItem {
  ...
  presensi: {
    ...
    catatan: string | null
    waktu_mulai_kelas: string | null
  } | null   // null = belum diabsen
}

// Payload batch upsert
export interface IBatchAbsenItem {
  id_siswa: string
  status: PresensiStatus
  catatan?: string | null
}

export interface IBatchAbsenPayload {
  id_jadwal: string
  items: IBatchAbsenItem[]
}
```

---

### Types — `src/@types/catat-kelas-siswa.types.ts`
...
### Service — `src/services/presensi.service.ts`
...
import type { ApiResponse, PaginatedResult } from '@/@types/api.types'
import type {
  IPresensi, IPresensiJadwalItem, IBatchAbsenPayload,
} from '@/@types/presensi.types'

export const presensiService = {
  // List semua presensi (pagination + filter bulan)
  getAll: (params?: { page?: number; limit?: number; search?: string; bulan?: string }) =>
    apiClient.get<{ data: IPresensi[]; meta: object }>(API_ENDPOINTS.presensi.list, { params }),

  // Absen list per sesi jadwal
  getByJadwal: (idJadwal: string) =>
    apiClient.get<ApiResponse<IPresensiJadwalItem[]>>(API_ENDPOINTS.presensi.byJadwal(idJadwal)),

  // Riwayat presensi + total sesi hadir satu siswa
  getBySiswa: (idSiswa: string) =>
    apiClient.get<{ data: IPresensi[]; sesi_terpakai: number }>(
      API_ENDPOINTS.presensi.bySiswa(idSiswa)
    ),

  // Presensi satu siswa di satu jadwal
  getBySiswaJadwal: (idJadwal: string, idSiswa: string) =>
    apiClient.get<ApiResponse<IPresensi | null>>(
      API_ENDPOINTS.presensi.bySiswaJadwal(idJadwal, idSiswa)
    ),

  // Absen massal (batch upsert)
  batchUpsert: (payload: IBatchAbsenPayload) =>
    apiClient.post<ApiResponse<IPresensi[]>>(API_ENDPOINTS.presensi.batch, payload),

  // Catat presensi satu siswa
  create: (payload: { id_jadwal: string; id_siswa: string; status: number; catatan?: string | null }) =>
    apiClient.post<ApiResponse<IPresensi>>(API_ENDPOINTS.presensi.list, payload),

  // Koreksi status presensi
  update: (id: string, payload: { status?: number; catatan?: string | null }) =>
    apiClient.patch<ApiResponse<IPresensi>>(API_ENDPOINTS.presensi.detail(id), payload),

  // Hapus presensi
  delete: (id: string) =>
    apiClient.delete<ApiResponse<null>>(API_ENDPOINTS.presensi.detail(id)),
}
```

---

### Service — `src/services/catat-kelas-siswa.service.ts`

```typescript
import apiClient from './api-client'
import { API_ENDPOINTS } from '@/constants/api.constant'
import type { ApiResponse } from '@/@types/api.types'
import type { ICatatKelasSiswa } from '@/@types/catat-kelas-siswa.types'

export const catatKelasSiswaService = {
  // Semua kelas yang diikuti siswa + total sesi hadir
  getBySiswa: (idSiswa: string) =>
    apiClient.get<ApiResponse<ICatatKelasSiswa[]>>(
      API_ENDPOINTS.catatKelasSiswa.bySiswa(idSiswa)
    ),

  // Semua siswa di kelas + total sesi hadir
  getByKelas: (idKelas: string) =>
    apiClient.get<ApiResponse<ICatatKelasSiswa[]>>(
      API_ENDPOINTS.catatKelasSiswa.byKelas(idKelas)
    ),
}
```
...
### Constants — tambahan di `src/constants/api.constant.ts`
...
export const API_ENDPOINTS = {
  ...
  presensi: {
    ...
    bySiswaJadwal:  (idJadwal: string, idSiswa: string) =>
                      `/kursus/presensi/siswa-jadwal/${idJadwal}/${idSiswa}`,
  },

  catatKelasSiswa: {
    bySiswa: (idSiswa: string) => `/kursus/catat-kelas-siswa/siswa/${idSiswa}`,
    byKelas: (idKelas: string) => `/kursus/catat-kelas-siswa/kelas/${idKelas}`,
  },
} as const
```
...
### Constants — tambahan di `src/constants/route.constant.ts`
...
export const ROUTES = {
  ...
  kursus: {
    ...
    presensiSesi:   (idJadwal: string) => `/kursus/presensi/${idJadwal}`,
  },
} as const
```
...
### Tampilan Kalender Presensi

Halaman `/kursus/presensi` menampilkan jadwal kelas dalam format **kalender mingguan** (Senin–Minggu) yang dikelompokkan per instruktur.

```
┌─────────────────────────────────────────────────────────────────┐
│  Absen Siswa                          [Filter instruktur...]    │
│                                                                 │
│  Instruktur: BUDI                                               │
│  ┌────────┬────────┬────────┬────────┬────────┬────────┬──────┐ │
...
Klik card jadwal → buka `AbsenSesiModal` → tampil list siswa + dropdown status per siswa → submit batch upsert.

#### Komponen `PresensiKalender.tsx`

- Terima props: `rows: { instruktur: string; jadwal: IJadwalKelas[] }[]`
- Render grid 7 kolom (Senin–Minggu)
- Tiap cell yang ada jadwal → render `PresensiSesiCard`

#### Komponen `AbsenSesiModal.tsx`

- Fetch `GET /kursus/presensi/jadwal/:id_jadwal` → dapat list siswa + status existing
- Render `AbsenSiswaRow` per siswa (dropdown status + input catatan)
- Submit → `POST /kursus/presensi/batch`
- Setelah sukses → backend otomatis update `catat_kelas_siswa`

#### Komponen `CatatKelasSiswaPanel.tsx`

Ditampilkan di halaman detail siswa. Fetch `GET /kursus/catat-kelas-siswa/siswa/:id_siswa` lalu tampilkan progress bar per kelas:
...
- `presensi: null` di response `byJadwal` = siswa **belum diabsen** di sesi itu, bukan berarti tidak hadir
- Gunakan `POST /batch` untuk absen massal — lebih efisien dari POST satu-satu
- `total_sesi_hadir` di `catat_kelas_siswa` diupdate **otomatis backend** — frontend cukup GET setelah batch sukses
- Refresh `CatatKelasSiswaPanel` setelah berhasil batch upsert untuk tampilkan angka terbaru
...
Kelas (memiliki id_paket langsung)
  └── KategoriUmur (id_kelas wajib)
        └── Biaya (jenis: PENDAFTARAN / KELAS / LAINNYA)
...
kursus: {
  ...
  siswaDaftar:              '/kursus/siswa/daftar',
...
// ============================================================
// BIAYA
// jenis_biaya: PENDAFTARAN / KELAS / LAINNYA
// ============================================================
...
// Pendaftaran siswa one-shot (POST /kursus/siswa/daftar)
export interface IPendaftaranTagihanItem {
  id_biaya: string           // wajib
  id_jadwal_kelas?: string
  periode?: string
  sesi_pertemuan?: number
}

export interface IDaftarSiswaPayload {
  // data siswa
  nama_siswa: string
  email?: string
  telepon?: string
  ...
  foto_url?: string
  // tagihan (min 1)
  tagihan: IPendaftaranTagihanItem[]
  // diskon — pilih SALAH SATU
  id_diskon?: string         // dari dropdown GET /kursus/diskon/aktif
  kode_diskon?: string       // input manual kode promo
}

export interface IDaftarSiswaResult {
  siswa: ISiswa
  tagihan: ITagihan[]
